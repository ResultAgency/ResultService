...
if($(document).ready()){
    ...
    var boxselect=$('#box');
    var boxname="Business";
    $('button[data-box]').on("click", function () {
        boxname = $(this).data('box');
    });
    $('#tea').on('show.bs.modal', function (e) {
        boxselect.val(boxname.toString());
    });
...
var f=true;
$('.how').appear();
$('.how').on('appear',function(){
    if(f) {
        $(this).attr('src', '../img/animation/how.gif');
        f=false;
    }
});
